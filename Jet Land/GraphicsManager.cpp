...
BOOL GraphicsManager::InitializeGraphicsSystem(UINT window_width, UINT window_height, BOOL enable_fullscreen, BOOL enable_vsync, UINT msaa_count, HWND h_window)
{
	...
	m_text = new DirectX::SpriteFont(renderManager_->GetDirectXDevice(), L"cn.font");
	m_spriteBatch = new DirectX::SpriteBatch(renderManager_->GetDirectXDeviceContext());
...
BOOL GraphicsManager::Render(Camera * camera)
{
	...
	renderManager_->GetDirectXDeviceContext()->OMSetDepthStencilState(NULL, 1);
	...
	auto e = std::wstring(L"FPS: ") + std::to_wstring(fps_->GetFps());

	//DirectX::SimpleMath::Vector2 origin = m_text->MeasureString(output) / 2.0f;

	m_text->DrawString(m_spriteBatch, e.c_str(), m_fontPos);
