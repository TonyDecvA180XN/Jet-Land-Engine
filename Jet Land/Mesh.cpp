...
VOID Mesh::Load(std::string * filename)
{
	if (filename->size() > 4)
	{
		...
		if (std::equal(extJMH.rbegin(), extJMH.rend(), filename->rbegin()))
		{
			LoadMeshJMHFromFile(filename, &vertexFormat_, &vertexArray_, &vertexCount_, &vertexSize_, &indexArray_, &indexCount_);
		}
		if (std::equal(extOBJ.rbegin(), extOBJ.rend(), filename->rbegin()))
		{
